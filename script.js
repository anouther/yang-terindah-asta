...
function pindahPosisi() {
  ...
  var parent = buttonNo.parentNode;
  parent.insertBefore(buttonYes, buttonNo.nextSibling);
}

function happy() {
  if (a < finish.length) {
    document.getElementById("textlv").innerHTML += finish.charAt(a);
    a++;
    setTimeout(happy, 120);
  }
  if (a == finish.length) {
    var e1 = document.getElementById('photo');
    e1.classList.add("degdeg");                                         
    document.getElementById('contTom').style.opacity = "1";
    document.getElementById('contTom').style.visibility = "visible";
  }
}
